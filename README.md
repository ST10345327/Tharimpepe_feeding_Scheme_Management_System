...
This serves the Apache-style PHP UI at `http://localhost:8080/` and the API at `http://localhost:8080/api`.
...
**Physical device:** the APK targets `192.168.18.73:8080` (current LAN IP). The phone must be on the same Wi-Fi network as the computer, and Windows Firewall must allow PHP on port `8080`. If the IP changes, update the host in `www/js/config.js` and `www/index.html`, run `npx cap copy android`, and rebuild:
...
The debug APK can be sent to teammates for testing. They can install it by enabling permission to install unknown apps for the app they use to open the APK. For the API to work, their phone must be on the same Wi-Fi as this computer while the PHP server is running; otherwise use a shared HTTPS server and update the API host before rebuilding.
